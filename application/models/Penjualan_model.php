...
class penjualan_model extends CI_Model
{
    ...
    {
        $query = $this->db->query("SELECT * FROM booking ORDER BY tanggal_reservasi DESC");
    ...
    {
        $query = $this->db->query("SELECT * FROM booking as b JOIN meja as m on b.id_meja = m.id_meja WHERE b.id_detail_menu = '$invoice'");
        return $query->row();
    }
    public function getTransaksiByInvoice($invoice)
    {
        $query = $this->db->query("SELECT * FROM menu_dibooking as md JOIN menu as m ON md.nama_makanan = m.nama_menu where md.id_detail_menu = '$invoice'");
        return $query->result_array();
    }
